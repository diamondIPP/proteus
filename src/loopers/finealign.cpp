...
{
  // Build a vector of sensor indices which will be permutated at eaich
  // iteration
  // fdibello@cern.ch convergence plot for the fine aligmenet variables, offset
  // x=0, offset y=1, offset z=2, rotation z=3
  ...
  TFile* out_file =
      new TFile("aligment_convergence_DUT_C22_masking_plane0.root", "RECREATE");
  TDirectory* sensordir[_refDevice->getNumSensors()];

  double rotation1[_refDevice->getNumSensors()]; // fdibello offset in the
                                                 // aligmnent plot
  ...
  for (unsigned int niter = 0; niter < _numIterations; niter++) {
    ...
    // Each sensor gets an unbiased residual run, and there is an extra run for
    // overall alignment
    for (unsigned int nsensor = 0; nsensor < _refDevice->getNumSensors();
         nsensor++) {
      ...
      Analyzers::Residuals residuals(
          _refDevice, 0, "", numPixX, binxPerPix, _numBinsY);
      ...
      for (ULong64_t nevent = _startEvent; nevent <= _endEvent; nevent++) {
        ...
        if (refEvent->getNumClusters())
          throw "FineAlign: can't recluster an event, mask the tree in the "
                "input";
        for (unsigned int nplane = 0; nplane < refEvent->getNumPlanes();
             nplane++)
      ...
      // Bilbao@cern.ch: first alignment using the residuals in order to avoid a
      // big ofset on the 2D technique. This also helps since the DUT is align
      // with repspect to a ref plane but not consdeing the cumulative shift.
      if (niter == 0) {
        double offsetX = 0, sigmaX = 0, maxX = 0, backgroundX = 0;
        double offsetY = 0, sigmaY = 0, maxY = 0, backgroundY = 0;
        ...
        Processors::residualAlignment(
            residuals.getResidualXY(nsens), // i guess this is a typo --->nsens
                                            // below instead of nsensor
        ...
        // " << sensor->getOffZ() <<",  RotationX="<<sensor->getRotX()<<",
        // RotationY="<<sensor->getRotY()<<",  RotationZ="<<sensor->getRotZ()<<
        // std::endl;
      }
      ...
      // Processors::fitGaussian(residuals.getResidualX(nsens), offsetX,
      // sigmaX1, maxX1,backgroundX1, _displayFits);
      // Processors::fitGaussian(residuals.getResidualY(nsens), offsetY,
      // sigmaY1, maxX1,backgroundY1, _displayFits);
  ...
  for (unsigned int i = 0; i < _refDevice->getNumSensors(); i++) {
    ...
    convergence[0][i] =
        new TGraphErrors(_numIterations,
                         nit,
                         ofxx,
                         0,
                         0); // how estimate error for the rotation?
    convergence[1][i] =
        new TGraphErrors(_numIterations,
                         nit,
                         ofyy,
