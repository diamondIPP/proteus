...
public:
  ...
  inline void setPix(double x, double y)
  {
    _pixX = x;
    _pixY = y;
  }
  ...
  inline void setPixErr(double x, double y)
  {
    _pixErrX = x;
    _pixErrY = y;
  }
  inline void setPosErr(double x, double y, double z)
  {
    _posErrX = x;
    _posErrY = y;
    _posErrZ = z;
...
private:
  ...
  Track* _matchedTrack; // Track matched to this cluster in DUT analysis (not
                        // stored)
