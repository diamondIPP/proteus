...
int mapping(char *input, char *output, const char *mapput = "mapping.root")
{
    ...
    if (!tsumIn)
        cout << "WARNING: no SummaryTree found in input file..." << endl;
    else
        ntrees += 1;
    if (!tEventIn)
    ...
    for (int plane = 0; plane < (f->GetNkeys() - ntrees); plane++) {
        ...
        // new Plane directory, new Hits tree, new Intercepts tree by cloning
        // Plane/Intercepts tree
        ...
        m_pltree->Branch("NHits", &unmapped.NHits,
                         "NHits/I");  // same as input
        m_pltree->Branch("PixX", mapped.PixX, "HitPixX[NHits]/I");
        m_pltree->Branch("PixY", mapped.PixY, "HitPixY[NHits]/I");
        m_pltree->Branch("Value", unmapped.Value,
                         "HitValue[NHits]/I");  // same as input
        m_pltree->Branch("Timing", unmapped.Timing,
                         "HitTiming[NHits]/I");  // same as input
        m_pltree->Branch("HitInCluster", unmapped.HitInCluster,
                         "HitInCluster[NHits]/I");  // same as input
        m_pltree->Branch("PosX", unmapped.PosX,
                         "HitPosX[NHits]/D");  // same as input
        m_pltree->Branch("PosY", unmapped.PosY,
                         "HitPosY[NHits]/D");  // same as input
        m_pltree->Branch("PosZ", unmapped.PosZ,
                         "HitPosZ[NHits]/D");  // same as input

        // new Plane directory for mapping histograms
        // someone please add axis title
        TDirectory *dmap = fmap->mkdir(name.Data());
        dmap->cd();
        TH2D *mapping_old = new TH2D("mapping_old", "mapping_old", 6, -0.5,
                                     6.5, 16, -0.5, 15.5);
        TH2D *mapping_new = new TH2D("mapping_new", "mapping_new", 12, -0.5,
                                     11.5, 8, -0.5, 7.5);
        TH2D *mapping_corX = new TH2D("mapping_corX", "mapping_corX", 6, -0.5,
                                      5.5, 12, -0.5, 11.5);
        TH2D *mapping_corY = new TH2D("mapping_corY", "mapping_corY", 16, -0.5,
                                      15.5, 8, -0.5, 7.5);
        ...
        for (int i = 0; i < nentries; ++i) {  // loop over entries
            ...
            for (int j = 0; j < unmapped.NHits; ++j) {  // loop over NHits
                if (unmapped.NHits > 0) {
                    if (minY % 2 == 0)
                        mapped.PixY[j] =
                            TMath::FloorNint((unmapped.PixY[j] /*+1*/) / 2.0);
                    else
                        mapped.PixY[j] =
                            TMath::FloorNint((unmapped.PixY[j] - 1.0) / 2.0);
                    // if((int)((unmapped.PixY[j]-minY)/2)%2!=0){
                    if ((int)((unmapped.PixX[j])) % 2 != 0) {
                        if ((unmapped.PixY[j]) % 2 != 0)
                            mapped.PixX[j] = 2 * unmapped.PixX[j] + 1;
                        if ((unmapped.PixY[j]) % 2 == 0)
                            mapped.PixX[j] = 2 * unmapped.PixX[j];
                    }
                    ...
                    /* }
                           if((int)((unmapped.PixY[j]-minY)/2)%2==0){
                                   if((unmapped.PixY[j])%2 != 0) mapped.PixX[j]
                       =2* unmapped.PixX[j];
                                   if((unmapped.PixY[j])%2 == 0) mapped.PixX[j]
                       =2* unmapped.PixX[j]+1;
                           }*/
                    mapping_old->SetBinContent(
                        int(unmapped.PixX[j] + 1),
                        int(unmapped.PixY[j] - minY + 1),
                        (int)(unmapped.PixY[j] - minY) * 6 +
                            (int)unmapped.PixX[j] + 1);
                    ...
                    mapping_new->SetBinContent(
                        int(mapped.PixX[j] + 1),
                        int(mapped.PixY[j] - (int)(minY / 2) + 1),
                        (int)(unmapped.PixY[j] - minY) * 6 +
                            (int)unmapped.PixX[j] + 1);
                    mapping_corX->Fill(int(unmapped.PixX[j]),
                                       int(mapped.PixX[j]));
                    mapping_corY->Fill(int(unmapped.PixY[j] - minY),
                                       int(mapped.PixY[j]) - (int)minY / 2);
                    //	cout <<" Entry " << i << " |X= " << unmapped.PixX[j] <<
                    //" --> " << mapped.PixX[j]<<" |Y= " << unmapped.PixY[j] <<
                    //" --> " << mapped.PixY[j] << endl;
                    //	cout <<" Entry " << minY << " |X= " <<
                    //int(unmapped.PixX[j]) << " --> " << mapped.PixX[j]<<" |Y=
                    //" << int(unmapped.PixY[j]-minY) << " --> " <<
                    //mapped.PixY[j] << endl;
                }
            }
            m_pltree->Fill();
