...
int Regions(std::string input, int bx0, int bx1, int Sy0, int Sy1, int Sa0, int Sa1 ) {
  ...
  if (f == 0) {
    cout<<"Cannot open, N.B. --No .root needed as input file--"<<in<<endl; 
  }
  else{cout<<"opening "<<in<<endl;}
  ...
  for (int i = 0; i < nentries; ++i) {
    ...
    for (int k = 0; k < read.NHits; ++k) {		
      if (read.PixX[k]>=bx0 && read.PixX[k]<=bx1 && read.PixY[k]>=Sy0 && read.PixY[k]<=Sy1 ) {
	flag+=1; 
	...
	out_ana0.PosZ[k] = read.PosZ[k];
      ...
      if (read.PixX[k]>=bx0 && read.PixX[k]<=bx1 && read.PixY[k]>=Sa0 && read.PixY[k]<=Sa1 ) {
	flag1+=1; 
